# Changelog - 07/07/2026 15:15
## Frase descritiva: Adicionando lições sobre arquitetura

Este changelog registra a modificação feita no arquivo de lições aprendidas (`lessons.md`) para documentar as decisões e aprendizados da arquitetura de geração estática de playlists de convidados.

### Prompt Motivador
> Desenvolvimento interno do motor do projeto: documentar as lições de projeto relacionadas à separação de parâmetros e variáveis CSS dinâmicas para futuras iterações.

### Explicação da Feature
- **Antes**: O arquivo `lessons.md` continha apenas considerações iniciais sobre a integração com Google Apps Script (tratamento de CORS) e prevenção de spam client-side usando `LocalStorage`.
- **Agora**: Adicionou-se uma nova seção contendo os aprendizados sobre arquitetura de geração estática (separação de variáveis no arquivo `config.js` dinâmico e aplicação de temas dinâmicos injetando cores hex diretamente em variáveis nativas do CSS em tempo de execução).

---

### Comparativo de Código

#### Código Antigo (`lessons.md`)
```markdown
