...
- **Funcionamento**: Processa requisições HTTP GET e POST externas. Utiliza a API nativa do Google Apps Script para manipular a planilha ativada nas abas `Musicas` e `Curtidas` com bloqueio concorrente (`LockService`). Suporta operações GET nativas de leitura e escrita para total compatibilidade com CORS.
- **Tópicos do Código**:
  - *Constante `CONFIG`*: Define nomes de abas e comprimento máximo.
  - *Função `doGet`*: Roteia a listagem de músicas (`action=list`), sugestão (`action=suggest`) e curtidas (`action=like`) via GET.
  - *Função `doPost`*: Recebe e parseia payloads JSON para cadastrar músicas (`action=suggest`) e votar (`action=like`) via POST.
...
- **Funcionamento**: Conecta a página HTML ao Apps Script do Google e implementa as validações de cliente. Comunica-se exclusivamente via GET para garantir 100% de compatibilidade com CORS.
- **Tópicos do Código**:
