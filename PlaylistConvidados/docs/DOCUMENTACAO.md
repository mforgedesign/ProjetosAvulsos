...
- **Funcionamento**: Processa requisições HTTP GET externas contendo a query string `action`. Utiliza a API nativa do Google Apps Script para manipular a planilha ativada.
- **Tópicos do Código**:
  - *Implantação & Instruções*: Comentários iniciais detalhando como implantar o Web App.
  - *Função `doGet`*: Roteador central que intercepta parâmetros e aciona métodos específicos.
  - *Função `ensureHeaders`*: Valida a existência dos cabeçalhos na primeira linha da planilha.
  - *Função `getSongsList`*: Lê todas as linhas de músicas e retorna um array de objetos JSON.
  - *Função `incrementLike`*: Localiza uma linha específica por ID e soma 1 na coluna `Likes`.
  - *Função `addNewSong`*: Verifica redundâncias de strings normalizadas e anexa nova linha de música.
  - *Funções Utilitárias*: `normalizeString` (remove acentos, espaços e caracteres especiais) e `createJsonResponse` (retorna JSON com cabeçalho de MimeType correto).
